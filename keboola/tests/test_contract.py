...
from connector_keboola.app import build_definition, create_runtime_app
from connector_keboola.service import ORIGINS, SLUG, VERSION, KeboolaService
...
def test_manifest_matches_runtime_definition() -> None:
    ...
    assert manifest["credentials"] == [
        ...
        for key in ("stack", "storage_token", "pii_key")
    ]
...
def test_signed_boundary_fails_closed_and_pseudonymises_provider_data() -> None:
    def upstream(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(200, json=[{"id": "in.c-secret", "name": "Secret bucket"}])
    ...
    with TestClient(runtime) as client:
        ...
        assert "in.c-secret" not in response.text
        ...
        for unknown in ("create_table", "run_job", "delete_bucket"):
