...
from connector_comgate.app import build_definition, create_runtime_app
from connector_comgate.service import ORIGIN, SLUG, VERSION, ComgateService
...
def test_manifest_matches_runtime_definition() -> None:
    ...
    assert manifest["credentials"] == [
        ...
        for key in ("merchant", "secret", "pii_key")
    ]
    ...
    assert ORIGIN == "https://payments.comgate.cz/v2.0"
...
def test_signed_boundary_fails_closed_and_pseudonymises_provider_data() -> None:
    def upstream(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200, json={"code": 0, "status": "PAID", "email": "secret@example.test"}
        )
    ...
    body = {
        ...
        "tool": TOOL,
        "arguments": {"trans_id": "AB12-CD34-EF56"},
    ...
    with TestClient(runtime) as client:
        ...
        for unknown in ("create_payment", "refund_payment", "cancel_payment"):
