"""Manifest drift and the real signed HTTP boundary of the Collabim runtime."""
...
from connector_collabim.app import build_definition, create_runtime_app
from connector_collabim.service import PATHS, SLUG, VERSION, CollabimService
...
def test_manifest_matches_runtime_definition() -> None:
    ...
    assert manifest["egress"] == {"host": "api.oncollabim.com", "port": 443, "methods": ["GET"]}
...
def test_signed_boundary_fails_closed_and_tool_returns_pseudonymized_data() -> None:
    ...
    def upstream(request: httpx.Request) -> httpx.Response:
        seen.append(request)
        return httpx.Response(200, json={"data": [{"id": 1, "name": "Private project"}]})
    ...
    body = {
        ...
        "secret_version": 1,
        "provider_credential": {"api_key": API_KEY, "pii_key": PII_KEY},
    }
    ...
    with TestClient(runtime) as client:
        ...
        headers = signed(body)
        tampered = body | {"provider_credential": {"api_key": "x", "pii_key": "changed"}}
        ...
        assert PII_KEY not in response.text and API_KEY not in response.text
        assert seen[-1].url.path == "/projects" and seen[-1].headers["authorization"] == API_KEY
        # A consumed token is never accepted twice.
        assert client.post(path, content=raw(body), headers=headers).status_code == 401
        for unknown in ("delete_all", "create_record", f"{TOOL}_write", "update_activity"):
