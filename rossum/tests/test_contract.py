...
from connector_rossum.app import build_definition, create_runtime_app
from connector_rossum.service import SLUG, VERSION, RossumService
...
def test_manifest_matches_runtime_definition() -> None:
    ...
    assert manifest["credentials"] == [
        ...
        for key in ("base_url", "api_token", "pii_key")
    ]
    ...
    assert manifest["egress"] == {
        "host_pattern": "{tenant}.rossum.app",
        "port": 443,
        "path_prefix": "/api/v1",
        "methods": ["GET"],
    }
...
def test_signed_boundary_fails_closed_and_pseudonymises_provider_data() -> None:
    def upstream(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200, json={"pagination": {"total": 1}, "results": [{"id": 8, "name": "Secret queue"}]}
        )
    ...
    with TestClient(runtime) as client:
        ...
        assert response.json()["result"]["data"]["results"][0]["id"] == 8
        assert "Secret queue" not in response.text
        assert PII_KEY not in response.text and "synthetic-token" not in response.text
        # A consumed token is never accepted twice.
        assert client.post(path, content=raw(body), headers=headers).status_code == 401
        for unknown in ("delete_annotation", "upload_document", "confirm_annotation"):
