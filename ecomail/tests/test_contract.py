...
from connector_ecomail.app import build_definition, create_runtime_app
from connector_ecomail.service import SLUG, VERSION, EcomailService
...
def test_manifest_matches_runtime_definition() -> None:
    ...
    assert manifest["credentials"] == [
        {"key": key, "required": True, "secret": True} for key in ("api_key", "pii_key")
    ]
    ...
    assert manifest["egress"] == {"host": "api2.ecomailapp.cz", "port": 443, "methods": ["GET"]}
...
def test_signed_boundary_fails_closed_and_pseudonymises_provider_data() -> None:
    def upstream(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(200, json=[{"id": 29, "name": "Secret list"}])
    ...
    with TestClient(runtime) as client:
        ...
        assert "Secret list" not in response.text
        assert PII_KEY not in response.text and "synthetic-api-key" not in response.text
        # A consumed token is never accepted twice.
        assert client.post(path, content=raw(body), headers=headers).status_code == 401
        for unknown in ("subscribe", "send_campaign", f"{TOOL}_write"):
