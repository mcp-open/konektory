...
from connector_smartemailing.app import build_definition, create_runtime_app
from connector_smartemailing.service import SLUG, VERSION, SmartemailingService

SIGNING_KEY = "synthetic-runtime-signing-key-0123456789abcdef"
PII_KEY = "synthetic-runtime-pii-key-0123456789abcdef"
TOOL = "list_contactlists"
...
def test_manifest_matches_runtime_definition() -> None:
    ...
    assert manifest["credentials"] == [
        ...
        for key in ("username", "api_key", "pii_key")
    ]
...
def test_signed_boundary_fails_closed_and_pseudonymises_provider_data() -> None:
    def upstream(request: httpx.Request) -> httpx.Response:
        ...
        payload = {"status": "ok", "meta": [], "data": [{"id": 1, "name": "Secret list"}]}
    ...
    with TestClient(runtime) as client:
        ...
        for unknown in ("create_contact", "send_newsletter", f"{TOOL}_write"):
