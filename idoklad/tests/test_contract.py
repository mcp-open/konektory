...
from connector_idoklad.app import build_definition, create_runtime_app
from connector_idoklad.service import SLUG, TOKEN_PATH, VERSION, IdokladService
...
def test_manifest_matches_runtime_definition() -> None:
    ...
    # POST exists only for the OAuth2 token exchange on the identity host.
    assert manifest["egress"] == {
        "hosts": ["api.idoklad.cz", "identity.idoklad.cz"],
        "port": 443,
        "methods": ["GET", "POST"],
    }
...
def test_signed_boundary_fails_closed_and_invokes_documented_reads() -> None:
    ...
    def upstream(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path == TOKEN_PATH:
            token = {"access_token": "private-access-token-0123456789", "token_type": "Bearer"}
            return httpx.Response(200, json=token)
        return httpx.Response(
            200,
            json={
                "Data": {"Id": 5, "Name": "Private agenda", "Contact": {"Email": "x@example"}},
                "Message": "",
                "StatusCode": 200,
                "ErrorCode": 0,
            },
    ...
    runtime = create_app(
        build_definition(IdokladService(transport=httpx.MockTransport(upstream))),
    ...
    with TestClient(runtime) as client:
        ...
        headers = signed(body)
        tampered = body | {"provider_credential": CREDENTIALS | {"client_secret": "changed"}}
        ...
        assert "Private agenda" not in response.text
        assert "x@example" not in response.text
        assert PII_KEY not in response.text
        assert CREDENTIALS["client_secret"] not in response.text
        assert [(r.method, r.url.host, r.url.path) for r in seen] == [
            ("POST", "identity.idoklad.cz", TOKEN_PATH),
            ("GET", "api.idoklad.cz", "/v3/Account/CurrentAgenda"),
        ]
        # A consumed token is never accepted twice.
        assert client.post(path, content=raw(body), headers=headers).status_code == 401
        for unknown in ("delete_all", "create_issued_invoice", "get_current_agenda_write"):
