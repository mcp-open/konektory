...
from connector_heureka.app import build_definition, create_runtime_app
from connector_heureka.service import ORIGINS, SLUG, VERSION, HeurekaService
...
def test_manifest_matches_runtime_definition() -> None:
    ...
    assert manifest["credentials"] == [
        ...
        for key in ("api_key", "country", "pii_key")
    ]
    ...
    assert manifest["egress"] == {
        "hosts": ["ssl.heureka.cz", "ssl.heureka.sk"],
        "port": 443,
...
def test_signed_boundary_fails_closed_and_pseudonymises_provider_data() -> None:
    def upstream(request: httpx.Request) -> httpx.Response:
        ...
        assert request.url.path.startswith("/api/cart/e9a09238d23fb504c663d06553ca4838/1/")
        return httpx.Response(
            200, json={"order_id": 1234, "status": 1, "internal_id": "secret-internal-id"}
        )
    ...
    with TestClient(runtime) as client:
        ...
        assert "secret-internal-id" not in response.text
        assert PII_KEY not in response.text
        assert "e9a09238d23fb504c663d06553ca4838" not in response.text
        # A consumed token is never accepted twice.
        assert client.post(path, content=raw(body), headers=headers).status_code == 401
        for unknown in ("set_order_status", "cancel_order", "send_invoice"):
