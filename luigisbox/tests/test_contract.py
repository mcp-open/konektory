...
from connector_luigisbox.app import build_definition, create_runtime_app
from connector_luigisbox.service import SLUG, VERSION, LuigisboxService

SIGNING_KEY = "synthetic-runtime-signing-key-0123456789abcdef"
PII_KEY = "synthetic-runtime-pii-key-0123456789abcdef"
TOOL = "trending_queries"
...
def test_manifest_matches_runtime_definition() -> None:
    ...
    assert manifest["credentials"] == [
        ...
        for key in ("tracker_id", "public_key", "private_key", "pii_key")
    ]
...
def test_signed_boundary_fails_closed_and_pseudonymises_provider_data() -> None:
    def upstream(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path == "/v1/content_export":
            return httpx.Response(200, json={"total": 1, "objects": [{"url": "/p/secret-1"}]})
        return httpx.Response(200, json=[{"title": "secret trending phrase"}])
    ...
    with TestClient(runtime) as client:
        ...
        assert "secret trending phrase" not in response.text
        assert PII_KEY not in response.text and "synthetic-private-key" not in response.text
        # A consumed token is never accepted twice.
        assert client.post(path, content=raw(body), headers=headers).status_code == 401
        for unknown in ("update_content", "delete_content", f"{TOOL}_write"):
        ...
        assert "secret-1" not in test_response.text
