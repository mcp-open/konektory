...
from connector_fapi.app import build_definition, create_runtime_app
from connector_fapi.service import SLUG, VERSION, FapiService
...
CREDENTIALS = {"username": "synthetic@example.test", "api_key": API_KEY, "pii_key": PII_KEY}
...
def test_manifest_matches_runtime_definition() -> None:
    ...
    assert [item["key"] for item in manifest["credentials"]] == ["username", "api_key", "pii_key"]
    ...
    assert manifest["egress"] == {"host": "api.fapi.cz", "port": 443, "methods": ["GET"]}
...
def test_signed_boundary_fails_closed_and_invokes_documented_reads() -> None:
    ...
    def upstream(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200,
            json={"forms": [{"id": 5, "name": "Private form", "path": "PRIVATE-path"}]},
        )

    runtime = create_app(
        build_definition(FapiService(transport=httpx.MockTransport(upstream))),
    ...
    with TestClient(runtime) as client:
        ...
        headers = signed(body)
        tampered = body | {"provider_credential": CREDENTIALS | {"api_key": "changed"}}
        ...
        for unknown in ("delete_all", "create_invoice", "list_forms_write"):
    ...
    assert [r.url.path for r in seen] == ["/forms", "/user"]
